...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: "无法解析修订请求" }, { status: 400 });
  }
  ...
  try {
    ...
    console.error("[createPromptRevision]", error);
    return NextResponse.json({ ok: false, error: "保存修订版本失败，请稍后重试" }, { status: 500 });
  }
}
