...
import { getModelConfigs } from "./actions";
...
export default async function ModelsPage() {
  ...
  return (
    <AppShell
      title="模型配置页"
      description="管理 OpenAI 或 OpenAI 兼容模型配置。支持新增、编辑、删除。"
    >
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_420px]">
        <section className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-2xl shadow-slate-950/50 sm:p-8">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.28em] text-cyan-400">模型配置列表区</p>
              <h2 className="mt-2 text-2xl font-semibold tracking-tight">已保存的模型配置</h2>
